...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    A_Tx = vanilla_matmul(A_T, x)
    gradient_first_term = [a + b for a, b in zip(Ax, A_Tx)]
    B_T = vanilla_transpose(B)
    B_Ty = vanilla_matmul(B_T, y)
    return [g1 + g2 for g1, g2 in zip(gradient_first_term, B_Ty)]
...
def numpy_solution(
    ...
    """
    gradient_first_term = np.dot(A, x) + np.dot(A.T, x)
    gradient_second_term = np.dot(B.T, y)
    return gradient_first_term + gradient_second_term
